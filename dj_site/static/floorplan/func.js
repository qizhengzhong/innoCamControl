...
function simulatePlan() {
    ...
    allAgents.forEach (function(value, key) {
      if (key.startsWith("robot"))
      {
        robotLocations.push(value.x + ","+ value.y);
        robotNames.push(key);
      }
      if (key.startsWith("buffer"))
      {
        bufferLocations.push(value.x + ","+ value.y);
        bufferNames.push(key);
      }
      if (key.startsWith("machine"))
      {
        machineLocations.push(value.x + ","+ value.y);
      }
    })
    let text = "{";
    if (machineLocations.length != 0)
    {
        text += "\"machineLocations\": [";
        for (i = 0; i < machineLocations.length - 1; i++) {
            text += "\"" + machineLocations[i] + "\",";
        }
        text += "\"" + machineLocations[machineLocations.length-1] + "\"]";
    }
    else
    {
        window.alert("You need at least one machine to do the simulation");
    }
    if (robotLocations.length != 0)
    {
        text += ",\n\"robotLocations\": [";
        for (i = 0; i < robotLocations.length - 1; i++) {
            text += "\"" + robotLocations[i] + "\",";
        }
        text += "\"" + robotLocations[robotLocations.length-1] + "\"]";
        text += ",\n\"robotNames\": [";
        for (i = 0; i < robotNames.length - 1; i++) {
            text += "\"" + robotNames[i] + "\",";
        }
        text += "\"" + robotNames[robotNames.length-1] + "\"]";
    }
    else
    {
        window.alert("You need at least one robot to do the simulation");
    }
    if (bufferLocations.length != 0)
    {
        text += ",\n\"bufferLocations\": [";
        for (i = 0; i < bufferLocations.length - 1; i++) {
            text += "\"" + bufferLocations[i] + "\",";
        }
        text += "\"" + bufferLocations[bufferLocations.length-1] + "\"]";
        text += ",\n\"bufferNames\": [";
        for (i = 0; i < bufferNames.length - 1; i++) {
            text += "\"" + bufferNames[i] + "\",";
        }
        text += "\"" + bufferNames[bufferNames.length-1] + "\"]";
    }
    else
    {
        window.alert("You need at least one buffer to do the simulation");
    }
    text += "}"
    console.log(text)
    ...
    a.download = 'simulatorJsonInput.json';
    a.href = url;
    a.click();
}
...
function carpentryCalc(classObj, typeObj, sizeObj, thickObj, dividerObj = 10) {
        ...
        if (typeObj === 'simpleStair') {
            ...
            for (let i = 1; i < dividerObj + 1; i++) {
                pushToConstruc(construc, "M " + (-sizeObj / 2) + "," + ((-thickObj / 2) + (i * heightStep)) + " L " + (sizeObj / 2) + "," +
                    ((-thickObj / 2) + (i * heightStep)), "none", "#", 'none');
            }
            construc.params.resizeLimit.width = {min: 40, max: 200};
    ...
    if (classObj === 'energy') {
        ...
        construc.params.rotate = false;
        if (typeObj === 'robot1') {
            pushToConstruc(construc, "m -20,-20 l 40,0 l0,40 l-40,0 Z", "#ff0", "#333", '');
            construc.push({
                'text': "Robot",
                'x': '0',
                'y': '0',
        ...
        if (typeObj === 'bufferlarge') {
            pushToConstruc(construc,  qSVG.circlePath(0, 0, 32), "#fff", "#000", '');
        ...
        if (typeObj === 'machinea') {
            pushToConstruc(construc, "m -20,-10 l 40,0 l0,20 l-40,0 Z", "#f00", "#333", '');
            //pushToConstruc(construc,  qSVG.circlePath(0, 0, 16), "#fff", "#000", '');
        ...
        if (typeObj === 'machineb') {
            pushToConstruc(construc, "m -20,-10 l 40,0 l0,20 l-40,0 Z", "#00f", "#333", '');
            //pushToConstruc(construc,  qSVG.circlePath(0, 0, 16), "#fff", "#000", '');
            //pushToConstruc(construc, "m-15,-4 l30,0", "none", "#333", '');
            //pushToConstruc(construc, "m-14,-8 l28,0", "none", "#333", '');
            //pushToConstruc(construc, "m-11,-12 l22,0", "none", "#333", '');
        ...
        if (typeObj === 'machinec') {
            pushToConstruc(construc, "m -20,-10 l 40,0 l0,20 l-40,0 Z", "#0f0", "#333", '');
